    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                title = {
                    Text(text = stringResource(id = R.string.minuman))
                },
        ...
        Column(
            ...
            MinumSearchBar(searchText = searchText, onSearchTextChange = setSearchText)
            Spacer(modifier = Modifier.height(16.dp))
            MinumanContent(Modifier.weight(1f), searchText)
        }
    }
}

@Composable
fun MinumanContent(modifier: Modifier, searchText: String) {
    val viewModel: MinumViewModel = viewModel()
    ...
    if (data.isEmpty()) {
        ...
        LazyColumn(
            ...
            items (data) { minuman ->
                if (minuman.nama.contains(searchText, ignoreCase = true) || minuman.stock.contains(searchText, ignoreCase = true)) {
                    MinumListItem(minuman = minuman) {
                        val pesan = context.getString(R.string.x_diklik, minuman.nama)
...
@Composable
fun MinumListItem(minuman: Minuman, onClick: () -> Unit) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() }
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            Image(
                painter = painterResource(minuman.gambarResId),
