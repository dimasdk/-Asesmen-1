...
const val KEY_NAMA_CLEAN= "nama"
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun BersihScreen(navController: NavHostController, id: String? = null) {
    val context = LocalContext.current
    val db = CleanDb.getInstance(context)
    val factory = CleanModelFactory(db.dao)
    val viewModel: BersihViewModel = viewModel(factory = factory)
    ...
    LaunchedEffect(true) {
        ...
        val data = viewModel.getClean(id) ?: return@LaunchedEffect
        if (data != null) {
            nama = data.nama
        }
        if (data != null) {
            ...
            TopAppBar(
                ...
                actions = {
                    ...
                    IconButton(onClick = {
                        ...
                        shareDataClean(context, message, selectedImageUri)
                    }) {
                        Icon(
                    ...
                    if (id != null) {
                        DeleteActionClean { showDialog = true }
                        CleaningAlertDialog(
...
@Composable
fun DeleteActionClean(delete: () -> Unit) {
...
private fun shareDataClean(context: Context, message: String, imageUri: Uri?) {
...
@Composable
fun BersihScreenPreview() {
    InventoryTokoTheme {
        BersihScreen(rememberNavController())
    }
}