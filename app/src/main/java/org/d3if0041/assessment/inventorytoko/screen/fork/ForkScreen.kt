...
import org.d3if0041.assessment.inventorytoko.screen.mandi.DisplayAlertDialog
...
const val KEY_NAMA_DAPUR= "nama"
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ForkScreen(navController: NavHostController, id: String? = null) {
    val context = LocalContext.current
    val db = DapurDb.getInstance(context)
    val factory = DapurModelFactory(db.dao)
    val viewModel: ForkViewModel = viewModel(factory = factory)
    ...
    LaunchedEffect(true) {
        ...
        val data = viewModel.getDapur(id) ?: return@LaunchedEffect
        if (data != null) {
            nama = data.nama
        }
        if (data != null) {
            ...
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = {
                        ...
                        if (selectedImageUri == null) {
                            ...
                            try {
                                ...
                                Log.e("ForkScreen", "Error updating or inserting", e)
                                Toast.makeText(
                                    context,
                    ...
                    IconButton(onClick = {
                        ...
                        shareDataDapur(context, message, selectedImageUri)
                    }) {
                        Icon(
                    ...
                    if (id != null) {
                        DeleteActionDapur {showDialog = true}
                        DisplayAlertDialog(
...
@Composable
fun DeleteActionDapur(delete: () -> Unit) {
...
private fun shareDataDapur(context: Context, message: String, imageUri: Uri?) {
...
@Composable
fun ForkScreenPreview() {
    InventoryTokoTheme {
        ForkScreen(rememberNavController())
    }
}