...
import org.d3if0041.assessment.inventorytoko.database.MakananDb
...
import org.d3if0041.assessment.inventorytoko.util.MakananModelFactory
...
fun MakananScreen(navController: NavHostController) {
    ...
    val factory = MakananModelFactory(db.dao)
    val viewModel: MakananViewModel = viewModel(factory = factory)
    var showBottomSheet by remember { mutableStateOf(false) }
    var selectedMakanan by remember { mutableStateOf<Makanan?>(null) }

    val dataStore = SettingsDataStore(LocalContext.current)
    val showList by dataStore.layoutFlow("makanan_layout").collectAsState(initial = true)
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                title = { Text(text = stringResource(id = R.string.makanan)) },
                ...
                actions = {
                    IconButton(onClick = {
                        CoroutineScope(Dispatchers.IO).launch {
                            dataStore.saveLayout(!showList, "makanan_layout")
                        }
                    }) {
                        Icon(
        ...
        Column(modifier = Modifier.padding(padding)) {
            ...
            MakananSearchBar(searchText, viewModel::setSearchText)
            ScreenContent(showList, modifier = Modifier.fillMaxSize(), navController, viewModel) { makanan ->
    ...
    if (showBottomSheet && selectedMakanan != null) {
        BottomSheetScaffold(
            sheetContent = {
                BottomSheetContente(selectedMakanan!!) {
...
@Composable
fun BottomSheetContente(makanan: Makanan, onClose: () -> Unit) {
    Column(modifier = Modifier.padding(16.dp)) {
        ...
        ItemMakanan(label = "Berat", value = "${makanan.berat}")
        Spacer(modifier = Modifier.height(20.dp))

        ItemMakanan(label = "Panjang", value = "${makanan.panjang}")
        Spacer(modifier = Modifier.height(20.dp))

        ItemMakanan(label = "Lebar", value = "${makanan.lebar}")
        Spacer(modifier = Modifier.height(20.dp))

        ItemMakanan(label = "Tinggi", value = "${makanan.tinggi}")
        Spacer(modifier = Modifier.height(20.dp))

        Spacer(modifier = Modifier.height(4.dp))
        Divider(modifier = Modifier.padding(vertical = 10.dp))

        ItemMakanan(label = "Deskripsi Produk", value = makanan.deskripsi, isDescription = true)
    }
}

@Composable
fun ItemMakanan(label: String, value: String, isDescription: Boolean = false) {
    if (isDescription) {
    ...
    if (data.isEmpty()) {
        ...
        if (showList) {
            LazyColumn(
                ...
                items(data) { makanan ->
                    ListItemMakanan(makanan = makanan, onClick = {
                        navController.navigate(Screen.FormUbahMakanan.withNama(makanan.nama))
            ...
            LazyVerticalStaggeredGrid(
                ...
                items(data) { makanan ->
                    GridItemMakanan(makanan = makanan, onClick = {
                        navController.navigate(Screen.FormUbahMakanan.withNama(makanan.nama))
                    }, onViewDetails = onViewDetails)
                }
            }
...
@Composable
fun ListItemMakanan(makanan: Makanan, onClick: (Makanan) -> Unit, onViewDetails: (Makanan) -> Unit) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Row(verticalAlignment = Alignment.Top) {
            ...
            Column {
                ItemMakanan1(label = "Nama", value = makanan.nama)
                ItemMakanan1(label = "Stok", value = makanan.stock)
                ItemMakanan1(label = "Harga", value = makanan.harga)
            }
        }
        Button(
            onClick = { onViewDetails(makanan) },
            modifier = Modifier
                .align(Alignment.End)
                .width(140.dp)
                .height(35.dp),
...
@Composable
fun MakananListMessage(modifier: Modifier) {
    Column(
        modifier = modifier.fillMaxSize(),
        verticalArrangement = Arrangement.Center,
        horizontalAlignment = Alignment.CenterHorizontally
...
@Composable
fun MakananScreenPreview() {
    InventoryTokoTheme {
        MakananScreen(rememberNavController())
    }
}